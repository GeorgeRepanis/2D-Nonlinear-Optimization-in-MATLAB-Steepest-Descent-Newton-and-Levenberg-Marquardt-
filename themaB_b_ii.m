...
% Αρχικό σημείο: (-1, -1), μη σταθερό γ_k (line search)
...
for k = 1:maxIter
    ...
    gamma_k = fminbnd(phi, 0, 5);   % διάστημα αναζήτησης π.χ. [0,5]
    % --------------------------------------------------------
...
x_star = xk;
f_star = f(xk);
fprintf('\nΤελικό σημείο: x* = (% .6f, % .6f), f(x*) = % .6f\n', ...
        x_star(1), x_star(2), f_star);
%% Διάγραμμα σύγκλισης f(x_k) vs k  (exact line search, x0 = (-1,-1))

k_axis = 0:length(Fhist)-1;   % επαναλήψεις k
